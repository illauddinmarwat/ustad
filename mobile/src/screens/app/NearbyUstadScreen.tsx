...
export default function NearbyUstadScreen() {
  const navigation = useNavigation<BottomTabNavigationProp<TabParamList, 'Nearby'>>();
    ...
    <View style={styles.screen}>
      ...
      >
        ...
        {filteredRows.map((worker) => (
          <WorkerCard key={worker.user_id} worker={worker} onCall={() => callWorker(worker.phone)} onBook={bookWorker} />
        ))}
      </ScrollView>
    </View>
  );
...
function WorkerCard({ worker, onCall, onBook }: { worker: NearbyWorker; onCall: () => void; onBook: () => void }) {
  const { t } = useT();
  ...
  return (
    <Card padding="lg">
      ...
      <View style={styles.actionRow}>
        ...
        />
        <Button labelId="nearby.card.book" onPress={onBook} iconLeft="calendar" hideUrdu style={styles.actionBtn} />
      </View>
    </Card>
  );
}
